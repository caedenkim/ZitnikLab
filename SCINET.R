...
for (i in t(unique(immune$obs["cell_ontology_class"]))) {
  ...
  if("cd8-positive, alpha-beta cytotoxic t cell" == i){
...
print(paired.datasets$genes[order(topo.spec.withRef, decreasing = TRUE)[1:10]]) #taking top 10 genes

print(paired.datasets$genes[order(topo.spec.withRef, decreasing = TRUE)[1:20]]) #taking top 20 genes
