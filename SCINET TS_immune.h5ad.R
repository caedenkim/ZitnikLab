...
for (i in t(unique(immune$obs["cell_ontology_class"]))) {
  ...
  if(grepl("cd8-positive, alpha-beta cytotoxic t cell", i)){ 
...
edge.idx = (G.adj@i)*nrow(G.adj) + (G.adj@j+1)
  ...
  immune_cells = construct_cell_networks(net = G.adj, gene_activities = paired.datasets$activity.scores[, 1:20], thread_no = 1)
  immune_cells
